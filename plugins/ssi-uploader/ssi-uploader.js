(function (root, factory) {
  ...
  Ssi_upload.prototype.init = function (element) {
    $(element)
      .addClass("ssi-uploadInput")
      .after((this.$element = $('<div class="ssi-uploader">')));
    var $chooseBtn = $(
      "" +
        '<span class="ssi-InputLabel">' +
        '<button class="ssi-button success">' +
        this.language.chooseFiles +
        "</button>" +
        "</span>"
    ).append(element);
    var $uploadBtn = $(
      '<button id="ssi-uploadBtn" class="ssi-button success ssi-hidden" >' +
        '<span class="ssi-btnIn">' +
        this.language.upload +
        "&nbsp;</span>" +
        '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>'
    );
    var $clearBtn = $(
      '<button id="ssi-clearBtn" class="ssi-hidden ssi-button info" >' +
        this.language.clear +
        "</button>"
    );
    var $abortBtn = $(
      '<button id="ssi-abortBtn" class="ssi-button error ssi-cancelAll ssi-hidden" ><span class="inBtn">' +
        this.language.abort +
        " </span></button>"
    );
    ...
    this.$element.append(
      $('<div class="ssi-buttonWrapper">').append(
        $chooseBtn,
        $abortBtn,
        $uploadBtn,
        $clearBtn
    ...
    if (!this.options.preview) {
      ...
      var $mainBox = $(
        '<div id="ssi-uploadFiles" class="ssi-tooltip ssi-uploadFiles ' +
          (this.options.dropZone ? "ssi-dropZone" : "") +
          '"><div id="ssi-uploadProgressNoPreview" class="ssi-uploadProgressNoPreview"></div></div>'
      ).append($namePreview);
      var $uploadDetails = $('<div class="ssi-uploadDetails"></div>').append(
        $fileList
      );
      $uploadBox = $(
        '<div class="ssi-uploadBoxWrapper ssi-uploadBox"></div>'
      ).append($mainBox, $uploadDetails);
      this.$element.prepend($uploadBox);
    } else {
      $uploadBox = $(
        '<div id="ssi-previewBox" class="ssi-uploadBox ssi-previewBox ' +
          (this.options.dropZone ? "ssi-dropZonePreview ssi-dropZone" : "") +
          '"><div id="ssi-info">' +
          (this.options.dropZone
            ? '<div id="ssi-DropZoneBack">' + this.language.drag + "</div>"
            : "") +
          '<div id="ssi-fileNumber" class="ssi-hidden">?</div></div></div>'
      );
    ...
    $clearBtn.click(function (e) {
      //choose files completed and pending files
      e.preventDefault();
      thisS.clear();
    });
    ...
    $uploadBox.on("mouseenter", "#ssi-fileNumber", function (e) {
      ...
      var message =
        " " +
        thisS.language.pending +
        ": " +
        thisS.pending +
        " <br> " +
        thisS.language.completed +
        ": " +
        (thisS.successfulUpload + thisS.aborted + thisS.abortedWithError) +
        "<br> " +
        thisS.language.inProgress +
        ": " +
        thisS.inProgress;
    ...
    $uploadBox.on("click", ".ssi-removeBtn", function (e) {
      ...
      if (thisS.options.preview) {
        ...
        if (thisS.currentListLength === 1) {
          //if only one file left in the list
      ...
      if (thisS.currentListLength === 0) {
        // if no more files in the list
        if (!thisS.options.dropZone) {
          // if drag and drop is disabled
  ...
  function tooltip($target, text, thisS) {
    ...
    if (!text) return;
    var $toolTip = $(
      '<div class="ssi-fadeOut ssi-fade ssi-tooltipText">' + text + "</div>"
  ...
  Ssi_upload.prototype.toUploadFiles = function (files) {
    ...
    if (this.inProgress === 0 && this.pending === 0) {
      //if no file are pending or are in progress
    ...
    var extErrors = [],
      ...
      errorMessage = "";
    var toUploadLength,
      filesLength = (length = toUploadLength = files.length);
    if (typeof this.options.maxNumberOfFiles === "number") {
      //check if requested files agree with our arguments
      if (
        filesLength >
        this.options.maxNumberOfFiles - (this.inProgress + this.pending)
      ) {
        //if requested files is more than we need
        filesLength = toUploadLength =
          this.options.maxNumberOfFiles - (this.inProgress + this.pending); // set variable to the number of files we need
      }
    }
    //
    for (var i = 0; i < filesLength; i++) {
      ...
      if ($.inArray(ext, this.options.allowed) === -1) {
        // if requested file not allowed
        if (length > filesLength) {
          //there are more file we dont pick
          filesLength++; //the add 1 more loop
        } else {
          toUploadLength--;
        }
        if ($.inArray(ext, extErrors) === -1) {
          //if we see first time this extension
          extErrors.push(ext); //push it to extErrors variable
        }
      } else if (
        (file.size * Math.pow(10, -6)).toFixed(2) > this.options.maxFileSize
      ) {
        //if file size is more than we ask
        if (length > filesLength) {
          filesLength++;
        } else {
          toUploadLength--;
        ...
        $.inArray(file.name, this.imgNames) === -1
      ) {
    ...
    if (extErrorsLength + sizeErrorsLength > 0) {
      // in the end expose all errors
      if (extErrorsLength > 0) {
        errorMessage = this.language.extError.replaceText(
          extErrors.toString().replace(/,/g, ", ")
        );
      }
      if (sizeErrorsLength > 0) {
        errorMessage += this.language.sizeError.replaceText(
          sizeErrors.toString().replace(/,/g, ", "),
          this.options.maxFileSize + "mb"
        );
      }
      this.options.errorHandler.method(
        errorMessage,
    ...
    function setupReader() {
      ...
      if (thisS.options.preview) {
        var getTemplate = function (content) {
          return (
            ...
            content +
            "</td></tr>" +
            '<tr><td><div id="ssi-uploadProgress' +
            index +
            '" class="ssi-hidden ssi-uploadProgress"></div></td></tr>' +
            '<tr><td><button data-delete="' +
            index +
            '" class=" ssi-button error ssi-removeBtn"><i class="fi fi-rr-cross"></i></button></td></tr>' +
            "<tr><td>" +
        ...
        if (fileType[0] == "image") {
          ...
          fileReader.onload = function () {
            imgContent += getTemplate(
              '<img class="ssi-imgToUpload" src=""/><i class="fa-spin fa fa-spinner fa-pulse"></i>'
            ); // set the files element without the img
            imgs[index] = fileReader.result;
            j++;
            if (toUploadLength === j) {
              // if all elements are in place lets load images
          ...
          $uploadBox.append(
            getTemplate(
              '<div class="document-item" href="test.mov" filetype="' +
                ext +
        ...
        $clearBtn.prop("disabled", false);
        thisS.$element
          .find(".ssi-namePreview")
          .html(
            index === 0
              ? cutFileName(filename, ext, 13)
              : thisS.currentListLength + 1 + " " + thisS.language.files
          ); //set name preview
        $fileList.append(
          '<tr class="ssi-space"><td></td></tr>' + //append files element to dom
            '<tr class="ssi-toUploadTr ssi-pending"><td><div id="ssi-uploadProgress' +
            index +
            '" class="ssi-hidden ssi-uploadProgress ssi-uploadProgressNoPre"></div>' +
            "<span>" +
            cutFileName(filename, ext, 20) +
            "</span></td>" +
            '<td><a data-delete="' +
            index +
            '" class="ssi-button ssi-removeBtn  ssi-removeBtnNP"><span class="trash7 trash"></span></a></td></tr>'
        );
      }
      ...
      var setImg = function () {
        ...
        for (var i = 0; i < imgs.length; i++) {
          if (imgs[i] !== null) {
            $uploadBox
              .find("#ssi-uploadProgress" + i)
              .parents("table.ssi-imgToUploadTable")
              .find(".ssi-imgToUpload")
              .attr("src", imgs[i]) //set src of the image
              .next()
              .remove(); //remove the spinner
            imgs[i] = null;
          }
        }
        imgs = [];
  ...
  Ssi_upload.prototype.clear = function (action) {
    ...
    if (this.inProgress === 0) {
      //if no file are uploading right now
      this.totalProgress = [];
    }
    if (this.currentListLength === 0) {
      ...
      if (!this.options.dropZone) {
        this.$element
          .find(".ssi-uploadBox")
          .removeClass("ssi-uploadNoDropZone");
      }
    }
    $clearBtn.prop("disabled", true);
    $uploadBtn.prop("disabled", true);
  ...
  var setNamePreview = function (thisS) {
    //set the name preview according to the remaining elements in the list
    if (thisS.currentListLength > 1) {
      //if more than one element left
      thisS.$element
        .find(".ssi-namePreview")
        .html(thisS.currentListLength + " files"); // set the name preview to the length of the remaining items
  ...
  Ssi_upload.prototype.uploadFiles = function () {
    ...
    if (this.pending > 0) {
      ...
      this.$element
        ...
        .children("span")
        .removeClass("trash7 trash10 trash")
      ...
      if (this.inProgress === this.currentListLength) {
        // disable the clear button if no items in list we can be remove
        $clearBtn.prop("disabled", true);
      }

      while (!thisS.toUpload[i]) {
        // do it until you find a file
        i++;
      }
    ...
    function ajaxLoopRequest(formData, ii) {
      ...
      uploadBar
        .removeClass("ssi-hidden") //make it visible
        .parents(selector)
        .removeClass("ssi-pending");
      var ajaxOptions = $.extend(
        {},
        {
          //store the request to the uploadList variable
          xhr: function () {
            var xhr = new window.XMLHttpRequest();
            xhr.upload.addEventListener(
              "progress",
              function (e) {
                // add event listener to progress
                if (e.lengthComputable) {
                  ...
                  var sum =
                    arraySum(thisS.totalProgress) /
                    (thisS.inProgress + thisS.successfulUpload); //and calculate the overall progress
                  if (!thisS.options.preview) {
                    thisS.$element
                      .find("#ssi-uploadProgressNoPreview")
          ...
          beforeSend: function (xhr, settings) {
            ...
            console.log(
              "TCL: ajaxLoopRequest -> thisS.toUpload",
              thisS.toUpload
            );
            console.log("TCL: ajaxLoopRequest -> ii", ii);
            $uploadBtn
              .find("#ssi-up_loading") //add spiner to uploadbutton
              ...
              try {
                ...
                if (err.name == "Error") {
                  ...
                  if (!thisS.options.ignoreCallbackErrors) {
                    console.log(
                      "There is an error in beforeEachUpload callback. Filename:" +
                        thisS.toUpload[ii].name
                    );
                    console.log(err);
            ...
            thisS.$element
              .find("input.ssi-uploadInput")
              .trigger("beforeEachUpload.ssi-uploader", [fileInfo]);
            if (xhr.status === 0) {
              if (xhr.statusText === "canceled") {
                //if user used beforeEachUpload to abort the request
            ...
            if (error !== "abort") {
              ...
              if (typeof thisS.options.onEachUpload === "function") {
                ...
                try {
                  ...
                  if (!thisS.options.ignoreCallbackErrors) {
                    console.log(
                      "There is an error in onEachUpload callback. File name:" +
                        thisS.toUpload[ii].name
                    );
                    console.log(err);
                  }
              ...
              if (getCompleteStatus(thisS)) {
                //if no more elements in progress
                finishUpload(thisS);
              }
      ...
      $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
        ...
        if (thisS.options.responseValidation) {
          ...
          if (
            typeof valData.validationKey === "object" &&
            valData.resultKey == "validationKey"
          ) {
        ...
        thisS.$element
          .find("input.ssi-uploadInput")
          .trigger("onEachUpload.ssi-uploader", [fileInfo]);
        thisS.inProgress--; //one less in progress upload
        $clearBtn.prop("disabled", false);
        if (getCompleteStatus(thisS)) {
          //if no more files in progress
          finishUpload(thisS);
        }
        // thisS.totalProgress[ii]='';
        thisS.uploadList[ii] = "";
      ...
      while (!thisS.toUpload[i] && typeof thisS.toUpload[i] !== "undefined") {
        // do it until you find a file
        i++;
      }
      if (i < thisS.toUpload.length) {
        // if more files exist start the next request
  ...
  var setElementMessage = function (thisS, index, msgType, msg, title) {
    ...
    element = thisS.$element.find(
      ".ssi-abortUpload[data-delete='" + index + "']"
    );
  ...
  var setLastElementName = function (thisS) {
    //if one file in list get the last file's name and put it to the name preview
    ...
    thisS.$element
      .find(".ssi-namePreview")
      .html(cutFileName(fileName, ext, 15)); //short the name and put it to the name preview
  };
  Ssi_upload.prototype.abort = function (index, title, mmsg) {
    ...
    if (typeof title === "undefined") {
      ...
      //if not string that means that the request aborted with the beforeEachUpload callback and no message returned
      title = "";
    }
    ...
    this.$element
      .find("#ssi-uploadProgress" + index)
      .removeClass("ssi-hidden")
      .addClass("ssi-canceledProgressBar");
    ...
    if (getCompleteStatus(this)) {
      //if no more file in progress
      finishUpload(this);
    }
  };

  var finishUpload = function (thisS) {
    //when every uplaod ends
    thisS.$element.find("#ssi-abortBtn").addClass("ssi-hidden");
    if (!thisS.options.preview) {
      //display tha main message in the name preview
      var type = "error",
        title = "",
        msg = "";
      ...
      thisS.$element
        .find(".ssi-namePreview")
        .append(getResultMessage(type, msg, title, "ssi-noPreviewMessage")); //show the message in the name preview
      thisS.$element
        .find("#ssi-uploadProgressNoPreview") //remove main overall progress bar
        .removeAttr("styles")
        .addClass("ssi-hidden");
    }
  ...
  $.fn.ssi_uploader = function (opts) {
    ...
    options.allowed = options.allowed || ["jpg", "jpeg", "png", "svg"];
  ...
  String.prototype.getExtension = function () {
    //returns the extension of a path or file
  ...
  var locale = {
    ...
    fa: {
      ...
      upload: "آپلود",
      clear: "پاک کردن",
      drag: "فایل خود را در اینجا بکشید و رها کنید",
      sizeError: "$1 حجم فایل زیاد است، حجم استاندارد $2 است",
      // $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
      extError: "$1 فایل پشتیبانی نمی شود",
      //$1=file extension ie(exe files are not supported)
      someErrorsOccurred: "برخی از خطاها رخ داده است!",
      wentWrong: "مشکلی پیش آمده است!",
      pending: "در انتظار",
      completed: "تکمیل شد",
